...
const fetchChildMenuItems = api => async () => {
  ...
  const selectionSet = wpStore.getState().remoteSchema.nodeQueries.menuItems.selectionSet;
  const query =
  /* GraphQL */
  `
  ...
  await Promise.all(remoteChildMenuItemNodes.map(async remoteMenuItemNode => {
    ...
    menuItemFetchQueue.add(fetchChildMenuItems(Object.assign(Object.assign({}, api), {}, {
      remoteNode: remoteMenuItemNode
    })));
    const type = buildTypeName(`MenuItem`);
    await actions.createNode(Object.assign(Object.assign({}, remoteMenuItemNode), {}, {
      nodeType: `MenuItem`,
      type: `MenuItem`,
      parent: null,
      internal: {
